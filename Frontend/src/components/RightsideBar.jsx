...
const RightsideBar = () => {
  ...
  return selectedUser && (
    <div className={` bg-gray-700 text-white w-1/4  relative overflow-y-scroll  rounded-r-lg ${selectedUser ? "max-md:hidden" : ""}`}>
      ...
      <button onClick={()=>logout()} className='absolute bottom-5 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-purple-500 to-violet-600 text-white px-20 py-2 rounded-full cursor-pointer font-light'>
        Logout
      </button>
    </div>
  )
